...
function initState() {
  ...
  if (isEmbedded && parentClient) {
    ...
    try {
      const saved = localStorage.getItem('email-audit-state');
      if (saved) {
      ...
      const savedNotes = localStorage.getItem('email-audit-notes');
      if (savedNotes) {
      ...
      const savedData = localStorage.getItem('email-audit-data');
      if (savedData) {
...
function saveState() {
  if (isEmbedded && parentClient) {
    ...
    try {
      localStorage.setItem('email-audit-state', JSON.stringify(state.checked));
      localStorage.setItem('email-audit-notes', JSON.stringify(state.notes));
      localStorage.setItem('email-audit-data', JSON.stringify({ targetUrl: state.targetUrl, textInputs: state.textInputs }));
    } catch (e) {}
  }
}
