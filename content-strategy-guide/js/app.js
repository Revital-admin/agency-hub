...
function initState() {
  ...
  if (isEmbedded && parentClient) {
    if (!parentClient.contentStrategy) {
      parentClient.contentStrategy = { checked: {}, notes: {}, targetUrl: "" };
    }
    if (!parentClient.contentStrategy.checked) {
      parentClient.contentStrategy.checked = {};
    }
    if (!parentClient.contentStrategy.notes) {
      parentClient.contentStrategy.notes = {};
    }
    // Bind child state object reference directly to the parent object
    Object.keys(parentClient.contentStrategy.checked).forEach(k => {
      if (k in state.checked) {
        state.checked[k] = parentClient.contentStrategy.checked[k];
      }
    });
    // Ensure all checklist keys are populated on parent client
    Object.keys(state.checked).forEach(k => {
      if (parentClient.contentStrategy.checked[k] === undefined) {
        parentClient.contentStrategy.checked[k] = state.checked[k];
      } else {
        state.checked[k] = parentClient.contentStrategy.checked[k];
      }
    });
    ...
    Object.keys(state.notes).forEach(k => {
      if (parentClient.contentStrategy.notes[k] === undefined) {
        parentClient.contentStrategy.notes[k] = "";
      }
      state.notes[k] = parentClient.contentStrategy.notes[k];
    });
    state.targetUrl = parentClient.contentStrategy.targetUrl || "";
  } else {
    try {
      const saved = localStorage.getItem('content-strategy-checklist-state');
      if (saved) {
      ...
      const savedNotes = localStorage.getItem('content-strategy-checklist-notes');
      if (savedNotes) {
      ...
      const savedUrl = localStorage.getItem('content-strategy-checklist-target-url');
      if (savedUrl) {
...
function saveState() {
  if (isEmbedded && parentClient) {
    parentClient.contentStrategy.checked = state.checked;
    parentClient.contentStrategy.notes = state.notes;
    parentClient.contentStrategy.targetUrl = state.targetUrl;
    window.parent.saveDatabase();
    window.parent.renderDashboard();
  } else {
    try {
      localStorage.setItem('content-strategy-checklist-state', JSON.stringify(state.checked));
      localStorage.setItem('content-strategy-checklist-notes', JSON.stringify(state.notes));
    } catch (e) {}
  }
}
  ...
  if (targetInput) {
    ...
    targetInput.addEventListener('input', e => {
      ...
      if (isEmbedded && parentClient) {
        ...
        try {
          localStorage.setItem('content-strategy-checklist-target-url', state.targetUrl);
        } catch (err) {}
      }
    });
  }
