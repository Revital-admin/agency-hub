...
async function persist() {
  if (isEmbedded && window.parent.saveVersionedAgencyDoc) {
    ...
    if (!result.ok) {
      if (result.reason === 'error') console.error("Couldn't save vendor/rental entry:", result.error);
...
function populateDatalists() {
  ...
  clientList.innerHTML = Object.keys(clients).filter(name => name !== SANDBOX_NAME).sort().map(name => `<option value="${name}">`).join('');
