...
const state = {
  ...
  textInputs: { budget: "", platform: "", notes: "", approvals: "" } // Paid Ads Inputs
};
...
function initState() {
  ...
  if (isEmbedded && parentClient) {
    if (!parentClient.campaignLaunch) {
      parentClient.campaignLaunch = { checked: {}, notes: {}, targetUrl: "", textInputs: { budget: "", platform: "", notes: "", approvals: "" } };
    }
    if (!parentClient.campaignLaunch.checked) {
      parentClient.campaignLaunch.checked = {};
    }
    if (!parentClient.campaignLaunch.notes) {
      parentClient.campaignLaunch.notes = {};
    }
    if (!parentClient.campaignLaunch.textInputs) {
      parentClient.campaignLaunch.textInputs = { budget: "", platform: "", notes: "", approvals: "" };
    }
    // Bind child state object reference directly to the parent object
    Object.keys(parentClient.campaignLaunch.checked).forEach(k => {
      if (k in state.checked) {
        state.checked[k] = parentClient.campaignLaunch.checked[k];
      }
    });
    // Ensure all checklist keys are populated on parent client
    Object.keys(state.checked).forEach(k => {
      if (parentClient.campaignLaunch.checked[k] === undefined) {
        parentClient.campaignLaunch.checked[k] = state.checked[k];
      } else {
        state.checked[k] = parentClient.campaignLaunch.checked[k];
      }
    });
    ...
    Object.keys(state.notes).forEach(k => {
      if (parentClient.campaignLaunch.notes[k] === undefined) {
        parentClient.campaignLaunch.notes[k] = "";
      }
      state.notes[k] = parentClient.campaignLaunch.notes[k];
    });
    state.targetUrl = parentClient.campaignLaunch.targetUrl || "";
    // Sync text inputs
...
function saveState() {
  if (isEmbedded && parentClient) {
    parentClient.campaignLaunch.checked = state.checked;
    parentClient.campaignLaunch.notes = state.notes;
    parentClient.campaignLaunch.targetUrl = state.targetUrl;
    parentClient.campaignLaunch.textInputs = state.textInputs;
...
function updateScoreCards() {
  ...
  setScoreValue('steps', `${doneSteps}/${STEPS.length}`);
...
function attachEvents() {
  ...
  list.addEventListener('input', e => {
    ...
    if (e.target.matches('input[id^="pa"]')) {
      ...
      if (id === 'clBudget') state.textInputs.budget = e.target.value;
      if (id === 'clPlatform') state.textInputs.platform = e.target.value;
      saveState();
    }
  });
  ...
  document.body.addEventListener('input', e => {
    if (e.target.matches('textarea[id^="pa"]')) {
      const id = e.target.id;
      if (id === 'clNotes') state.textInputs.notes = e.target.value;
      if (id === 'clApprovals') state.textInputs.approvals = e.target.value;
      saveState();
    }
  });
