...
**Third guard (Aug 2026):** the two guards above only fire around a *save*. There's also a pure read-side version of the same staleness: a tab left backgrounded for a long stretch can have its Firestore listeners and Firebase auth token quietly go stale without self-healing, showing an out-of-date `clientsDb` (a client added elsewhere just doesn't appear) and/or falling back to a "Sign in with Google" popup instead of the Hub's normal silent re-auth. `handleTabBecameVisible()` (a `visibilitychange` listener near the idle-lock code) now forces a fresh ID token and a fresh read of every shard the instant a backgrounded tab becomes visible again, instead of waiting on a listener that may not have noticed anything changed.

**Second guard (Aug 2026):** the shard-load guard above only protects a tab that never finished loading. It doesn't catch a tab that finished loading once, then went stale — e.g. sat backgrounded/throttled long enough that its per-shard listener quietly stopped receiving updates while its much cheaper meta-doc listener (which only tracks a version number) kept ticking along fine. That desync (version looks current, content isn't) is exactly what silently dropped Evry Intention LLC a second time even with the first guard in place. `commitDatabaseToCloud()` now does a second check right before writing: fetch the real shard documents fresh and compare their client keys against what this tab is about to write. Any client the cloud has that this tab doesn't (and that this tab didn't itself just delete/rename via `intentionallyRemovedClientNames`) blocks the save with the same "Reload Now" conflict banner used for a version mismatch, rather than silently overwriting.
